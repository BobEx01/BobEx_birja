...
class Database:
    ...
    async def foydalanuvchi_qoshish(self, user_id, username):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO foydalanuvchilar (user_id, username) VALUES ($1, $2) ON CONFLICT DO NOTHING",
                user_id, username
            )
    ...
    async def raqam_olingan_qoshish(self, user_id, elon_id, elon_turi, sanasi):
        async with self.pool.acquire() as conn:
            await conn.execute('''
                ...
                VALUES ($1, $2, $3, $4)
            ''', user_id, elon_id, elon_turi, sanasi)async def raqam_olingan_soni_yuk(self, elon_id):
        async with self.pool.acquire() as conn:
            await conn.execute(
    ...
    async def foydalanuvchi_mavjudmi(self, user_id):
        async with self.pool.acquire() as conn:
            result = await conn.fetchrow(
                "SELECT 1 FROM foydalanuvchilar WHERE user_id = $1", user_id
            )
            return bool(result)
    ...
    async def tolov_log_qoshish(self, user_id, summa, sana, izoh):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO tolov_log (user_id, summa, sana, izoh) VALUES ($1, $2, $3, $4)",
                user_id, summa, sana, izoh
            )

    async def foydalanuvchilar_soni(self):
        async with self.pool.acquire() as conn:
            result = await conn.fetchval("SELECT COUNT(*) FROM foydalanuvchilar")
            return result
